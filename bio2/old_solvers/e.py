...
def run(config_file):
    ...
    while ((j<config['max_inner_iterations']) and 
    (outer_difference>config['outer_iteration_target'])):
        j += 1
        ...
        (inner_difference>config['inner_iteration_target'])):
            i += 1
            old_v = v.copy()
            A_xx = A_x(**get_dict())
            if (A_xx<0).any():
    ...
    if j==config['max_inner_iterations']:
