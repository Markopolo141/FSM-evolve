...
genetic_symbols = ['+','-'] # note: must be reversed order under >
invert_key = {"+":"-","-":"+"}
homozygous_positive = "++"
hetrozygous = "+-"
homozygous_negative = "--"
...
def sex_propagate(old,coords,from_t,to_t):
    ...
    for other in index_combinations:
        ...
        # mono young arent pairing
        if other_d['age'] == "Y" and other_d['strategy'] == "Mono":            pass
        else:
            ...
            s = get_symbol(other)
            weighting += s
            v += sel_fact*s*reproduce_likelihood(from_t,other_d,to_t)
    return v/weighting
