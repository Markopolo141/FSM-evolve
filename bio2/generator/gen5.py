...
def age_propagate(old,coords,from_t,to_t):
    if from_t['age']=="Y" and (to_t['age']=="O" or to_t['age']=="P") and False not in [from_t[c] == to_t[c] for c in coords_keys_bar_age]:
        pass
    else:
        return old
    if from_t['sex']=='F':
        if from_t['genetics'] == homozygous_positive:
            genetic_factor = 0.8 #1
        elif from_t['genetics'] == hetrozygous:
            genetic_factor = 0.85 #1
        elif from_t['genetics'] == homozygous_negative:
            genetic_factor = 0.05 #1
    else:
        if from_t['genetics'] == homozygous_positive:
