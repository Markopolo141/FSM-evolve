...
\abstract{
Evolutionary scenarios can have significant complexity that is not always amenable to simple mathematical modeling.
To extend the complexity of what can be readily modelled mathematically, we consider the broad setting of organisms with state dependant strategic dynamics.
Particularly, organisms with strategic actions affecting the vital rates between states in the context of arbitrary non-linear population matrix models. We outlay formalism, present theorems, and and give an algorithm to solve for population-mean mixed-strategy equilibria by an iterated matrix-purturbation and power-iteration method.
We highlight the complexity and power of our modelling and solving software, by presenting and solving a model of the sexual selection of a deleterious mutation with heterozygous advantage, in the context of a variable sexual diamorphism, and between resource states.
The model encompases 24 states with 54 stretegic dimensions yielding $ 2^{54} $ pure strategies with non-linear coupled fitness values.
We apply our software algorithm to interogate the equilibrium and disequilibrium dynamics of such a game across a range of parameters.
}
